...
public class BCIConnector_Myo1_Data_ForcePercent : MonoBehaviour {
	...
	void Update () {
		...
		if (Myo_script && Myo_Paired) {
			...
			for (int i = 0; i < Myo_script.emg.Length; i++) {
				...
				if (emg_value_abs > Myo_1_emg_max)
					Myo_1_emg_max = emg_value_abs;
			}
			
			Myo_ForceHistory_values.Add (Myo_1_emg_max);
			Myo_ForceHistory_times.Add (Time.time);

            //Debug.Log(Myo_1_emg_max);
            ...
            if (Myo_ForceHistory_times [0] + HistoryTimeSec < Time.time) {
				...
				string DatasLinesToSync = "BCI_Myo_1:ForcePercent:" + Myo_Force_Percent + ":float";
                //Debug.Log(DatasLinesToSync);

                if (MyoForceData_Player1)
                {
                    MyoForceData_Player1.GetComponent<Graph_MyoForce_Player1>().EMG_value = Myo_Force_Percent;
                }
